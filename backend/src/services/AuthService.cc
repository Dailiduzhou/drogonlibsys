...
std::optional<TokenPair> AuthService::login(const std::string &username,
    ...
    if (!verifyPassword(user->password, password)) return std::nullopt;
...
std::optional<TokenPair> AuthService::refresh(const std::string &refreshToken) {
    JwtClaims claims;
    if (!JwtUtils::verify(refreshToken, claims)) return std::nullopt;
    if (RedisClient::isBlacklisted(claims.jti)) return std::nullopt;
...
bool AuthService::logout(const std::string &token) {
    JwtClaims claims;
    if (!JwtUtils::verify(token, claims)) return false;
