...
TokenPair JwtUtils::issue(int64_t userId, const std::string &username,
    ...
    pair.accessExpiresAt =
        std::chrono::duration_cast<std::chrono::seconds>(
            (now + std::chrono::seconds(g_accessTtl)).time_since_epoch()).count();
    pair.refreshExpiresAt =
        std::chrono::duration_cast<std::chrono::seconds>(
            (now + std::chrono::seconds(g_refreshTtl)).time_since_epoch()).count();
    return pair;
}
...
int JwtUtils::remainingTtl(const std::string &token) {
    try {
        ...
        auto now = std::chrono::system_clock::now();
        auto secs = std::chrono::duration_cast<std::chrono::seconds>(exp - now).count();
