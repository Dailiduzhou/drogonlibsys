...
void RedisClient::init(const std::string &host, int port, const std::string &password) {
    if (g_ctx) {
        redisFree(g_ctx);
        g_ctx = nullptr;
    }
    ...
    if (g_ctx == nullptr || g_ctx->err) {
        throw std::runtime_error(
            std::string("Redis connect failed: ") +
            (g_ctx ? g_ctx->errstr : "alloc error"));
    }
    if (!password.empty()) {
        auto *r = static_cast<redisReply *>(redisCommand(g_ctx, "AUTH %s", password.c_str()));
        if (r) {
            freeReplyObject(r);
        }
    }
...
bool RedisClient::isBlacklisted(const std::string &jti) {
    ...
    auto *r = static_cast<redisReply *>(redisCommand(g_ctx, "EXISTS %s", jti.c_str()));
...
bool RedisClient::set(const std::string &key, const std::string &value, int ttlSeconds) {
    ...
    if (ttlSeconds > 0) {
        r = static_cast<redisReply *>(
            redisCommand(g_ctx, "SET %s %s EX %d", key.c_str(), value.c_str(), ttlSeconds));
    } else {
        r = static_cast<redisReply *>(
...
std::optional<std::string> RedisClient::get(const std::string &key) {
    ...
    auto *r = static_cast<redisReply *>(redisCommand(g_ctx, "GET %s", key.c_str()));
...
bool RedisClient::del(const std::string &key) {
    ...
    auto *r = static_cast<redisReply *>(redisCommand(g_ctx, "DEL %s", key.c_str()));
    bool ok = r && r->type != REDIS_REPLY_ERROR;
    if (r) freeReplyObject(r);
    return ok;
}
