...
Book rowToBook(const drogon::orm::Row &row) {
    ...
    b.description = row["description"].isNull() ? "" : row["description"].as<std::string>();
    b.coverKey = row["cover_key"].isNull() ? "" : row["cover_key"].as<std::string>();
...
void PgClient::init() {
    g_db = drogon::app().getDbClient("pg");
}
...
bool PgClient::createUser(const std::string &username, const std::string &passwordHash,
...
std::vector<Book> PgClient::listBooks(int offset, int limit) {
    ...
    std::vector<Book> out;
    out.reserve(r.size());
    for (const auto &row : r) out.push_back(rowToBook(row));
    return out;
}
...
std::vector<Book> PgClient::search(const std::string &query, int offset, int limit) {
    auto f = g_db->execSqlAsyncFuture(
        "SELECT id, title, author, description, cover_key, stock, "
        "to_char(created_at, 'YYYY-MM-DD\"T\"HH24:MI:SS') AS created_at, "
        "to_char(updated_at, 'YYYY-MM-DD\"T\"HH24:MI:SS') AS updated_at, "
