...
int main() {
  try {
    ...
    // JWT 配置
    libsys::JwtUtils::init(config.jwt.secret, config.jwt.issuer,
                           config.jwt.accessTtlSeconds,
                           config.jwt.refreshTtlSeconds);

    // Redis 配置 (JWT 黑名单 + 业务缓存, hiredis 直连)
    libsys::RedisClient::init(config.redis.host, config.redis.port,
    ...
    // MinIO 对象存储
    libsys::MinioClient::init(config.minio.endpoint, config.minio.accessKey,
                              config.minio.secretKey, config.minio.bucket,
                              config.minio.secure);
